...
ERA5 fornece dados de ondas em tempo r    def create_quick_        print(\"🎨 Criando prévia dos dados ERA5...\")
        ...
        try:
            ...
            titles = ['Altura Significativa de Ondas (m)', 'Direção Média das Ondas (°)', 
                     'Período Médio (s)', 'Período de Pico (s)']s):
        ...
        try:
            ...
            variables = ['swh', 'mwd', 'mwp', 'pp1d']
            titles = ['Altura Significativa de Ondas (m)', 'Direção Média das Ondas (°)', 
                     'Período Médio (s)', 'Período de Pico (s)']Altura significativa de ondas (sw    if ds is not None:
        ...
        print("🚀 Próximo passo: criar visualizações combinadas SWOT + ERA5")o média das ondas (mwd)
...
class ERA5WaveDownloader:
    ...
    def check_copernicusmarine_installation(self):
        """Verifica se o copernicusmarine está instalado."""
        try:
            result = subprocess.run(['copernicusmarine', '--version'], 
                                  capture_output=True, text=True)
            if result.returncode == 0:
                version = result.stdout.strip()
                print(f"✅ copernicusmarine instalado: {version}")
                return True
            else:
                print("❌ copernicusmarine não está instalado corretamente")
                return False
        except FileNotFoundError:
            print("❌ copernicusmarine não encontrado")
            return False
    
    def install_copernicusmarine(self):
        """Instala o copernicusmarine se necessário."""
        print("📦 Instalando copernicusmarine...")
        try:
            subprocess.run([sys.executable, '-m', 'pip', 'install', 'copernicusmarine'], 
                          check=True)
            print("✅ copernicusmarine instalado com sucesso!")
            return True
        except subprocess.CalledProcessError as e:
            print(f"❌ Erro ao instalar copernicusmarine: {e}")
            return False
    ...
    def download_era5_waves(self, start_date, end_date):
        ...
        try:
            ...
            # Renomear variáveis para nomes mais simples
            var_mapping = {
            ...
            # Aplicar renomeação se variáveis existem
            rename_dict = {}
    ...
    # REMOVIDO: Função de dados sintéticos removida permanentemente
    # Apenas dados reais do Copernicus Marine Service são permitidos
    
    def create_quick_preview(self, ds):
        """Cria uma prévia rápida dos dados WAVERYS."""
        print("🎨 Criando prévia dos dados WAVERYS...")
        
        # Selecionar timestep no meio do período
        mid_time = ds.time[len(ds.time)//2]
        data_slice = ds.sel(time=mid_time)
        ...
        # Usar nomes das variáveis do WAVERYS real
        variables = ['VHM0', 'VMDR', 'VTPK', 'VTM02']
        titles = ['Altura Significativa de Ondas (m)', 'Direção Média das Ondas (°)', 
                 'Período de Pico (s)', 'Período Médio (s)']
        ...
        for i, (var, title, cmap) in enumerate(zip(variables, titles, cmaps)):
            ...
            # Configurar mapa
            ax.set_title(f'{title}\\n{mid_time.dt.strftime("%Y-%m-%d %H:%M").values} UTC')
            ax.gridlines(draw_labels=True, alpha=0.5)
            ax.set_extent([ds.longitude.min(), ds.longitude.max(),
                          ds.latitude.min(), ds.latitude.max()], ccrs.PlateCarree())
        
        plt.tight_layout()
        
        # Salvar figura
        preview_file = self.output_dir / 'waverys_preview.png'
        plt.savefig(preview_file, dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"🖼️ Prévia salva em: {preview_file}")
